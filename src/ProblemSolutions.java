...
public class ProblemSolutions {
    ...
    public static int lastBoulder(int[] boulders) {
        ...
        // Smashing the two heaviest boulders till one or none remains
        while (maxHeap.size() > 1) {
            int first = maxHeap.poll(); //  boulder
            int second = maxHeap.poll(); // Second boulder

            if (first != second) {
                maxHeap.add(first - second); // Add the result of the smash if not equal
            }
        }
    ...
    public static ArrayList<String> showDuplicates(ArrayList<String> input) {
        ...
        // Get strings that appear more than once
